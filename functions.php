 ...
 * @param PDO $pdo
 * 
 * @return array if the relation is not empty, else false
 */
...
function  saveCar( PDO $pdo, $id, $producer, $type,$vintage,$fuel )
{
 
    try
    {
            $smt = $pdo->prepare( "UPDATE `cars` SET `producer` = :producer, `type` = :type, `vintage` = :vintage, `fuel` = :fuel WHERE `cars`.`id` = :id;" );
            $smt->bindParam(':producer', $producer);
            $smt->bindParam(':type', $type);
            $smt->bindParam(':vintage', $vintage);
            $smt->bindParam(':fuel', $fuel);
            $smt->bindParam(':id', $id);
        $smt->execute();        
